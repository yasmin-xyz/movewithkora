...
COUNTER-POSE / RESET BREAKS (critical): When a block contains 2 or more poses from the same family/theme taught back-to-back (e.g. multiple hip-openers in a row: Pigeon Pose, Low Lunge, Lizard Pose), insert a brief neutral reset pose — most commonly Child's Pose — after every 2-3 such poses before continuing deeper into that theme, rather than stacking every pose from a family in one uninterrupted run. This reset still needs its own full Breath/Cue/Modifications like any other pose (see NO BARE POSE ENTRIES above) and counts as a real pose filling the block's Duration (see TIMING REALISM below). This applies across styles but matters most in Hatha and Yin, where the slower pace makes a genuine reset moment a real part of the teaching, not an afterthought.
...
- Sun Salutations should NOT be the very first thing a student does in class. Precede them with one brief grounding/mobility block first — 2-3 simple poses such as Child's Pose, Cat-Cow, or a gentle seated or tabletop warm-up — before moving into the Sun Salutation block(s). This grounding block still belongs in WARM-UP, just before the Sun Salutation block(s) rather than after. Real classes ease students in before dynamic movement; jumping straight into Sun Salutation A/B with zero preparation doesn't reflect how this is actually taught. A good default for this grounding block is Table Top building into Bird-Dog (Right/Left) — the added stability work gives Warm-Up more real substance without over-complicating it.
